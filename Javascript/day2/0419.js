...
function update() {
    ...
    // obt.left  = obtPosX 
    // obt.right  = obtPosX + (obtWidth)
    // obt.top  = obtPosY - (obtWidth)
    // obt.bottom  = obtPosY + (obtWidth)

    if(isCollisionRectToRect(ball, obt)){
        arcMoveDirY *= -1;
        // console.log('aaa')
        // obtPosY = obt.top - obtWidth;
    }

    // if(obtPosX - obtWidth < 0) {
    //     obtMoveDirX = 1
    // } else if (obtPosX + arcRadius > canvas.width) {
    //     obtMoveDirX = -1
    // }

    // obtPosX += obtMoveDirX * obtMoveSpeed; // 원 좌우로 움직이기. 
