...
import {getCoinbaseTransaction , getTransactionPool, processTransaction, getUnspentTxOuts ,updateTransactionPool } from './transaction.js'
...
const createNextBlock = () => {
    ...
    // 2. 생성된 코인베이스 트렌젝션 뒤에 현재 보유 중인 트랜잭션 풀의 내용을 포함(마이닝된 블록의 데이터)
...
const addBlock = (newBlock, previousBlock) => {
    if(isValidNewBlock(newBlock, previousBlock)) {
        ...
        // 트랜잭션 풀 업데이트
        updateTransactionPool(unspentTxOut);
...
const replaceBlockchain = (receiveBlockchain) => {
    ...
    if (isValidBlockchain(receiveBlockchain))
    {
        ...
        if((receiveBlockchain.length > blocks.length) ||
            receiveBlockchain.length == blocks.length.boolean())
        {
            console.log('받은 블록체인 길이가 길다');
            blocks = receiveBlockchain;

            // 사용되지 않은 txOuts 셋팅 , 잔액
            const latestBlock = getLatestBlock();
            processTransaction(getLatestBlock().data , getUnspentTxOuts(), latestBlock.index)
